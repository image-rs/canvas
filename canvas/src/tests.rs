use crate::{
    canvas::{ArcCanvas, PlaneDataMut, RcCanvas},
    color::Color,
    layout::{
...
fn color_conversion() -> Result<(), LayoutError> {
    ...
    // Note: tests below presume we convert with clamping as into gamut mapping.

    // colour-science says this should be: array([246.83446829, -18.2111931 ,  50.6162153 ])
    check_color_pair([255, 255, 0], [247, 0, 51]);
    // colour-science says: array([ 246.94351388,  -17.7958723 ,   49.63342989])
    check_color_pair([255, 255, 30], [247, 0, 50]);
    // colour-science says: array([ 180.39263341,   68.27850811,  -42.20367881])
    check_color_pair([255, 30, 255], [180, 68, 0]);
    // colour-science says: array([ 231.23355565,  -37.44856119,   -9.92645447])
    check_color_pair([30, 255, 255], [231, 0, 0]);
    // colour-science says this should be: array([100.55198341,  41.28176379,  -6.22791988])
...
#[test]
fn rgb_conversion() -> Result<(), LayoutError> {
    struct TestCase {
        rhs: Color,
        lhs: Color,
        rgb: &'static [([u8; 3], [u8; 3])],
        name: &'static str,
    }

    fn test(case: TestCase) -> Result<(), LayoutError> {
        let layout = CanvasLayout::with_texel(&Texel::new_u8(SampleParts::Rgb), 32, 32)?;
        let mut from = Canvas::new(layout.clone());
        from.set_color(case.lhs)?;

        let layout = CanvasLayout::with_texel(&Texel::new_u8(SampleParts::Rgb), 32, 32)?;
        let mut into = Canvas::new(layout);
        into.set_color(case.rhs)?;

        let mut check_color_pair = |rgb: [u8; 3], result: [u8; 3]| {
            from.as_texels_mut(<[u8; 3] as image_texel::AsTexel>::texel())
                .iter_mut()
                .for_each(|b| *b = rgb);
            ...
            into.as_texels_mut(<[u8; 3] as image_texel::AsTexel>::texel())
                .iter()
                .enumerate()
                .for_each(|(idx, b)| assert_eq!(*b, result, "at {} ({})", idx, case.name));
        };

        for (lhs, rhs) in case.rgb {
    ...
    let cases = [
        ...
        TestCase {
            lhs: Color::SRGB,
            rhs: Color::RGB_ITU_BT2020,
            rgb: &[
                // array([ 249.47792601,  253.55886066,   76.1270598 ])
                ([255, 255, 0], [249, 254, 76]),
                // array([ 154.485926  ,  246.11952502,  252.92011088])
                ([0, 255, 255], [154, 246, 253]),
                // array([ 208.90958078,   64.88646938,  243.61140021])
                ([255, 0, 255], [209, 65, 244]),
            ],
            name: "sRGB to BT.2020",
        },
        // As generated by:
        // ```python
        // import colour
        // colour.RGB_to_RGB(
        //  [180./255., 10./255., 180/255.],
        //  colour.RGB_COLOURSPACES['sRGB'],
        //  colour.RGB_COLOURSPACES['ITU-R BT.470 - 625']
        //  apply_cctf_decoding=True,
        //  apply_cctf_encoding=True,
        // ) * 255
        // ```
        TestCase {
            lhs: Color::SRGB,
            rhs: Color::RGB_ITU_BT470_625,
            // The gamut is smaller, so we test some colours that are in it.
            rgb: &[
                // array([ 192.70737515,  192.69719799,   91.3881768 ])
                ([180, 180, 70], [193, 193, 91]),
                // array([  65.44703153,  192.70256748,  192.69752928])
                ([10, 180, 180], [65, 193, 193]),
                // array([ 189.78763531,   32.00693681,  193.50876795])
                ([180, 10, 180], [190, 32, 194]),
            ],
            name: "sRGB to BT.470-625",
        },
    ];

    cases.into_iter().map(test).collect()
}
...
#[test]
fn to_data_conversion() -> Result<(), LayoutError> {
    let texel = Texel::new_u8(SampleParts::RgbA);
    ...
    let into_plane = target_layout.plane(0).unwrap();
    let mut data = vec![0; target_layout.byte_len()];
    let into = PlaneDataMut::new(&mut data, into_plane).unwrap();
    ...
    // Expecting conversion [0xff, 0xff, 0x0, 0xff] to 0–ff—ff
    {
        ...
        let mut plan = converter.plan(source_layout, target_layout).unwrap();
        plan.add_plane_in(from.plane(0).unwrap()).set_as_color();
        plan.add_data_out(into).set_as_color();
        plan.run().unwrap();
    }

    let data = <[u8; 2] as image_texel::AsTexel>::texel()
        .try_to_slice(&data[..])
        .unwrap();
